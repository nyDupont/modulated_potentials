...
def mechanical_energy_pendulum_amp_phase_mod_adim(param, *args):
    vect_t, a, dphi = args
    x_sol_adim, p_sol_adim = resolution(pendulum_amp_phase_mod_adim, (0, 0), vect_t, param, a=a, dphi=dphi)
    ...
    if abs(x_sol_adim[-1]) > pi:  # checking that the particle has indeed changed site
        ...
        great_energy = 2 ** 100  # a great amount of "energy" to force the passage in a next cell when minimizing energie_adim
        # should I use numpy.inf ?
